...
df <- read.csv("../Covid_study/220317_2000hiv_study_export_processed.csv", sep=";")
df <- dplyr::select(df, c("RecordId", "COVID_VACC_TYPE"))
colnames(df) <- c("ID", "Vac_type")

pheno3=readRDS("../../2Pheno/Phe_dis_clean_PCs.rds")
pheno3 <- merge(pheno3, df, by = "ID", all=T)
pheno3 <- dplyr::filter(pheno3, !is.na(pheno3$ID_idat))
...
pheno5 <- pheno4 %>% dplyr::select("ID", "AGE", "SEX_BIRTH", "Sample_Plate", "season_sin", "season_cos", "cov", "vac", "iso2", "cplv", "SMOKING", "BMI_BASELINE", "Vac_type", "CD8T", "CD4T", "NK", "Bcell", "Mono", "Neu", "PC1", "PC2", "PC3", "PC4", "PC5")
table(pheno5$vac)
pheno5[which(pheno5$Vac_type==0), "mRNA_vac"] = 1
pheno5[which(pheno5$Vac_type==1), "mRNA_vac"] = 1
pheno5[which(pheno5$Vac_type==2), "mRNA_vac"] = 0
pheno5[which(pheno5$Vac_type==4), "mRNA_vac"] = 0
pheno5[which(pheno5$Vac_type==6), "mRNA_vac"] = 0
pheno5[which(is.na(pheno5$vac)), "mRNA_vac"] = NA
#pheno5[which(pheno5$mRNA_vac!=1), "vac"] = NA

pheno5 <- dplyr::filter(pheno5, !is.na(pheno5$mRNA_vac))
...
for (i in 1:length(pheno6$ID)) {
    ...
    INR = dplyr::filter(pheno6, pheno6$ID==id)[, "mRNA_vac"] 
    if (is.na(INR)) {
...
rm(beta2)
rm(beta3)
save(Log,file="Outlier_log_mRNAvacOthers.Rdata") #save log
  ...
  tryCatch(
      {
        ML = rlm(methylation ~ cyto1 + AGE + SEX_BIRTH + Sample_Plate + season_sin + season_cos +
               CD8T + CD4T + NK + Bcell + Mono + Neu + PC1 + PC2 + PC3 + PC4 + PC5, 
               data=test.df)
...
saveRDS(cor_info,"corEWASrlm_dis_mRNAVacOthers_All.rds",compress="xz")
