...
pheno5 <- dplyr::filter(pheno5, !is.na(pheno5$cov))
...
for (i in 1:length(pheno6$ID)) {
    ...
    INR = dplyr::filter(pheno6, pheno6$ID==id)[, "cov"] 
    if (is.na(INR)) {
...
rm(beta2)
rm(beta3)
save(Log,file="Outlier_log_covAll.Rdata") #save log
...
saveRDS(cor_info,"corEWASrlm_dis_cov_All.rds",compress="xz")
