...
pheno5 <- dplyr::filter(pheno5, !is.na(pheno5$iso2))
...
for (i in 1:length(pheno6$ID)) {
    ...
    INR = dplyr::filter(pheno6, pheno6$ID==id)[, "iso2"] 
    if (is.na(INR)) {
...
rm(beta2)
rm(beta3)
save(Log,file="Outlier_log_isoAll.Rdata") #save log
...
saveRDS(cor_info,"corEWASrlm_dis_iso_All.rds",compress="xz")
